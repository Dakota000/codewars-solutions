...
Notes:
    1. The calculator only works for positive integers only.
    2. The calculator rounds any non-integer before operating.
    3. The calculator rounds any output to nearest integer.
...
function stickyCalc(operation, val1, val2){
    ...
    let stickyNumber = Number(`${val1}${val2}`);
    
    let result;
    
    if(operation === '+'){
      result = stickyNumber + val2;
    }else if(operation === '-'){
      result = stickyNumber - val2;
    }else if(operation === '*'){
      result = stickyNumber * val2;
